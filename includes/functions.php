	...
	if( isset( $bovp_sets->furl ) ) {
		...
		if( isset( $_SERVER['REQUEST_URI'] ) AND !empty( $_SERVER['REQUEST_URI'] ) ) {
			...
			if ( preg_match( '/log/', $bovp_set_params ) ) {
				...
				$bovp_sets->key_log = $explode_params[1];
...
function bovp_form_donation() {
	...
	echo "<div id='icone-title'><div class='icones-heart icone-left'></div><div class='title'>" . __( 'Donation', 'bovp' ) . "</div></div>";
...
<div id="bovp_header_adm" class="clearfix">
<img src="<?php echo $img_folder; ?>logo.png">
<h2 class="bovp_adm_h2">by vivendoapalavra.org</h2>
</div>
...
<div class="bovp_box">
	...
	<div id="icone-title">
	<div class="icones-setup icone-left"></div>
	<p class="title"><?php _e('Settings','bovp');?></p>
	</div>
			...
			<div class="bovp_item-setting">
				...
				<span><?php _e('Choose and Install the Bible version','bovp');?></span>
				        ...
				        <select name="bovp_user_settings[translate]" id="bovp_install">
					        ...
					        if ($bovp_sets->translate =='not_set') {

					            echo "<option value='not_set' selected>" . __('Not Installed','bovp') . "</option>";

					        } else {

					          	echo "<option value='not_set'>" . __('Not Installed','bovp') . "</option>";

					        }
				        
				            foreach( $bovp_sets->translate_info as $indice => $name ) {

								echo "<option value='" . $indice . "'"; 

								if ( $bovp_sets->translate === $indice ) {echo " selected";}
...
function bovp_init_page() {
	...
	<div class="bovp_box">
		...
		<div id="icone-title">
		<div class="icones-info icone-left"></div>
		<p class="title"><?php _e('Informations','bovp');?></p>
		</div>
		...
		<div class="icones-author icone-left"></div>
		<p class='float-left-icone'>Author:&nbsp;<a href="https://www.facebook.com/andrebrumsampaio">Andre Brum Sampaio</a></p>

		<div class="icones-link icone-left"></div>
		<p class='float-left-icone'><?php echo __('Author URI: ','bovp') . '&nbsp;<a href="http://www.vivendoapalavra.org/">http://www.vivendoapalavra.org/</a>' ?></p>
		
		<div class="icones-designer icone-left"></div>
		<p class='float-left-icone'><?php echo __('Theme Designer: ','bovp'); ?>Lucas Tolle </p>


		<div class="icones-version icone-left"></div>
		<p class='float-left-icone'><?php echo __('Version: ','bovp') .  $bovp_sets->version; ?></p>      

		<h3 class="bovp_adm_h3"><?php _e('Bible Translates:','bovp'); ?></h3>

		<div class="icones-book icone-left"></div><p class='float-left-icone'><?php _e('English Bible.','bovp'); ?></p>
		<div class="icones-book icone-left"></div><p class='float-left-icone'><?php _e('Portuguese Bible.','bovp'); ?></p>
		<div class="icones-book icone-left"></div><p class='float-left-icone'><?php _e('Spanish Bible.','bovp'); ?></p>
		<div class="icones-book icone-left"></div><p class='float-left-icone'><?php _e('Italian Bible.','bovp'); ?></p>
		<div class="icones-book icone-left"></div><p class='float-left-icone'><?php _e('Indonesian Bible.','bovp'); ?></p>
...
function bovp_set_info(){
	...
	$translates['pt_BR'] = __('Portuguese Translation - ACF -1994', 'bovp');
	$translates['en'] = __('English Translation - King James', 'bovp');
	$translates['es'] = __('Spanish Translation - RV 1960', 'bovp');
	$translates['id_ID'] = __('Bahasa Indonesia Translation', 'bovp');
	$translates['it'] = __('Italian Translation', 'bovp');

	$bovp_sets->translate_info = $translates;
